...
crit.barplot.AIC = qplot(x = model, y = AIC, fill = 'red',
  ...
  geom_bar(stat="identity", colour="white") + coord_cartesian(ylim=c(1550, 1580))

crit.barplot.BIC = qplot(x = model, y = BIC, fill = 'blue',
                         main = "BIC", xlab = '', ylab = "",
                         data = criteria, geom = 'bar', stat = 'identity',
                         position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat="identity", fill="#799AFF", colour="white") + coord_cartesian(ylim=c(1600, 1640))

crit.barplot.Mallow = qplot(x = model, y = Mallow, fill = 'red',
                            main = "Mallow's CP", xlab = '', ylab = "",
                            data = criteria, geom = 'bar', stat = 'identity',
                            position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat = "identity", fill="#FFB445", colour="white") + coord_cartesian(ylim=c(10, 20))

crit.barplot.adjr = qplot(x = model, y = adj.r.squared, fill = 'red',
                         main = "Adjusted R-squared", xlab = '', ylab = "",
                         data = criteria, geom = 'bar', stat = 'identity',
                         position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat="identity", fill = "#FF9494", colour="white") + coord_cartesian(ylim=c(.7430,.7440))

crit.barplot.CV = qplot(x = model, y = CV, fill = 'red',
                        main = "LOO Cross-Validation Score", xlab = '', ylab = "",
                        data = criteria, geom = 'bar', stat = 'identity',
                        position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat = "identity", fill="#6ADA6A", colour="white") + coord_cartesian(ylim=c(11450, 11550))

crit.barplot.GCV = qplot(x = model, y = GCV,
                        main = "General Cross-Validation Score", xlab = '', ylab = "",
                        data = criteria, geom = 'bar', stat = 'identity',
                        position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat = "identity", fill="#5CBD7D", colour="white") + coord_cartesian(ylim=c(11280, 11300))
...
# plot(bost$latdist, bost$medv, pch = 19, cex = .5)
# abline(lm(bost$medv ~ bost$latdist), col = 'red')

qplot(x=latdist, y=medv, data = bost, geom = c("point", "smooth"), method = "lm", formula = y ~ x)
