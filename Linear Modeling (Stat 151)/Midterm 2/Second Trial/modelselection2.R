...
limits = apply(criteria[,-1], 2, function(x) {
  dif = max(x) - min(x)
  return(c(min(x) - dif, max(x) + dif))
})
...
crit.barplot.AIC = qplot(x = model, y = AIC, fill = 'red',
  ...
  geom_bar(stat="identity", colour="white") + coord_cartesian(ylim=c(limits[1,1], limits[2,1]))

crit.barplot.BIC = qplot(x = model, y = BIC, fill = 'blue',
                         main = "BIC", xlab = '', ylab = "",
                         data = criteria, geom = 'bar', stat = 'identity',
                         position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat="identity", fill="#799AFF", colour="white") + coord_cartesian(ylim=c(limits[1,2], limits[2,2]))

crit.barplot.Mallow = qplot(x = model, y = Mallow, fill = 'red',
                            main = "Mallow's CP", xlab = '', ylab = "",
                            data = criteria, geom = 'bar', stat = 'identity',
                            position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat = "identity", fill="#FFB445", colour="white") + coord_cartesian(ylim=c(limits[1,3], limits[2,3]))

crit.barplot.adjr = qplot(x = model, y = adj.r.squared, fill = 'red',
                         main = "Adjusted R-squared", xlab = '', ylab = "",
                         data = criteria, geom = 'bar', stat = 'identity',
                         position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat="identity", fill = "#FF9494", colour="white") + coord_cartesian(ylim=c(limits[1,5], limits[2,5]))

crit.barplot.CV = qplot(x = model, y = CV, fill = 'red',
                        main = "LOO Cross-Validation Score", xlab = '', ylab = "",
                        data = criteria, geom = 'bar', stat = 'identity',
                        position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat = "identity", fill="#6ADA6A", colour="white") + coord_cartesian(ylim=c(limits[1,4], limits[2,4]))

crit.barplot.GCV = qplot(x = model, y = GCV,
                        main = "General Cross-Validation Score", xlab = '', ylab = "",
                        data = criteria, geom = 'bar', stat = 'identity',
                        position = 'dodge') + theme(legend.position="none") +
  geom_bar(stat = "identity", fill="#5CBD7D", colour="white") + coord_cartesian(ylim=c(limits[1,6], limits[2,6]))
