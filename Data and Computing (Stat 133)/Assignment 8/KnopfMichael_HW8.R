...
### If I were forced to choose one k-value, I would choose 4.  Running several trials of the program,
### each time recording the k-value with the lowest misclassification rate, gave a mode of 4.  Although
### the mean and median were about 6, I would argue that, for this situation, the mode is the most
...
### If we plot for k = 1:1000, the graph actually shows that, around k=300, the misclassification rate
### begins to level out and asymptotically approach .2562189, which is the proportion of emails in the set
### which are spam.  This makes sense, since as k approaches the number of emails in the training set, every
### email in the test set will be marked false, since the majority of the emails in the training set will be false.
### So the misclassification rate is the proportion of emails which should have been marked true: .2562189.

### An interesting feature of the graph, which also reflected itself in this 100-element sample,
### was that even k-values are more successful than odd k-values.  In fact, the most successful k-value
...
### If this program were actually used as a spam filter, it be better not to weight the votes
### equally.  The first few votes should weigh more heavily than the later ones.  We might want
...
### even out.  It would probably be best to actually weight the votes according to their absolute distance
...
# }
# 
# trials = replicate(n = 100, testRun())
