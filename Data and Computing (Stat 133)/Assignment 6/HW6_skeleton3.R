...
spamFilter = function(email, max_exclam = 10**5, max_up_body = 1, max_up_sub = 1){
  ...
  if (!is.na(reply_to) && grepl("<|>", reply_to)) {
    reply_to = strsplit(reply_to, "<|>")[[1]][2]
  }
...
test = function(){
  for(i in 1:length(Emails))
  {
    ...
    spamFilter(Emails[[i]], 100, 1, 1)
    #     if (is.na(Emails[[i]]$body[[1]]) | length(Emails[[i]]$body[[1]]) == 1) print(i)
  }
}

test2 = function(){
  given = sapply(Emails, function(x) x$spam)
  spams = sapply(Emails, spamFilter)
  return(mean(spams == given))
}
